...
<div class="flex-grow border h-100 main">
        ...
        <div class="taskbar flex items-center justify-between px-6 py-3.5 mb-4">
            <div class="flex items-center gap-4">
                <div class="flex gap-2 items-center">
                    ...
                    class="font-semibold">Operation log</span>
                </div>
            </div>
            <a href="{{ route('export.operations') }}" class="border px-3 hover-bg-c-black hover-text-c-yellow text-sm text-black py-1 rounded border-gray-600 btn btn-success" id="my_export" target="_blank">
    ...
    <button id="filter"
    class="custom-safety-btn rounded px-6 py-1 mr-1 hover:border-yellow-300 hidden">
    Filter
</button>
...
<div class="dropdown inline-block relative">
  <button class="flex items-center custom-safety-btn rounded px-6 py-1 mr-1 hover:border-yellow-300 data1" id="custom">
    <span class="mr-1">Select Tag</span>
    <svg class="fill-current h-4 w-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
      <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z" />
  </svg>
</button>
<ul class="dropdown-menu custom-dropdown-menu absolute hidden text-gray-700 shadow bg-custom-pure-white text-xs">
    <li>
            ...
            <table class="table-auto w-full">
                <thead>
                    <tr class="bg-c-dark-gray">
                        ...
                        <th class="py-2 text-white text-left pl-3">Action</th>
                ...
                <tbody class="border" id="log-entries" id="userList">
                    ...
                    <tr class="text-sm border-t">
                        ...
                        <td class="py-2 pl-3"><span class="flex items-center">
                            <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 10 10" fill="none" class="mr-1">
                                @if($logs->action == "Log In")
                                    <circle cx="5" cy="5" r="5" fill="#36C752"/>
                                @elseif($logs->action == "Log Out")
                                    <circle cx="5" cy="5" r="5" fill="#f7d200"/>
                                @endif
                            </svg>
                            {{ $logs->action }}
                        </span></td>
                        ...
                        @php
                            ...
                            preg_match('/\b(Windows \d+\.\d+|Mac OS X \d+_\d+|Linux|Ubuntu|Fedora|Debian|CentOS)\b.*?\b(Chrome \d+\.\d+\.\d+\.\d+|Firefox \d+\.\d+|Safari \d+\.\d+|Edge \d+\.\d+|Brave \d+\.\d+|Opera \d+\.\d+)\b/', $details, $matches);

                            $os = $matches[1] ?? 'Unknown OS';
                            $browser = $matches[2] ?? 'Unknown Browser';
                        @endphp
...
<script>
    $(document).ready(function(){
        $('.filter-button').on('click', function(){
            ...
            var baseUrl = "{{ url('export-operation') }}";
