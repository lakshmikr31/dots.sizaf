...
<div class="bg-white rounded-2xl overflow-hidden shadow-lg max-w-xl w-full bg-c-lighten-gray modal-content">
    ...
    <div class="p-5 overflow-y-auto scroll" style="max-height: calc(100vh - 10rem)">
        <form class="flex flex-col gap-4 text-sm" id="editgroup" action="{{ route('client.company.group.update', ['id' => $groupdetail->id]) }}" method="PUT">
            @csrf
                    ...
                    <select id="client_id" name="client"
                        ...
                        @foreach($clients as $client)
                            <option value="{{ base64_encode($client->id) }}" 
                                {{ $groupdetail->client_id == $client->id ? 'selected' : '' }}>
                                {{ $client->name }}
                            </option>
                        @endforeach
                    </select>
                    ...
                    <select id="company_id" name="company"
                        ...
                        @foreach($companies as $company)
                            <option value="{{ base64_encode($company->id) }}" 
                                {{ $groupdetail->company_id == $company->id ? 'selected' : '' }}>
                                {{ $company->name }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-10 gap-4 items-start">
                <div class="md:col-span-2 flex items-center">
                    <label for="groupname" class="block font-bold text-c-black">
                ...
                <div class="md:col-span-8">
                    <input id="group_name"
                        ...
                        type="text" value="{{ $groupdetail->name }}" placeholder="Please enter a group name" autocomplete="groupname" name="group_name"
                         data-validate="group_name" />
                    <small class="text-red-500 mt-1 block"></small>
                </div>
            </div>
